...
/**
 * 判断是否登录
 * @param {String} id vueX的getUserID 
 * @returns 是否登录的Boolean值, true: 已登录， false:未登录
 */
async function isLogin(id) {
  if(!id){
    return false
  }
  let status = !!window.localStorage.getItem('kuajingtongstatu') && 
    window.localStorage.getItem('kuajingtongstatu') == 'nologin'
  //不存在nologin的情况
  if(status){
    return false
  } else {
